...
def ner_test(ip):
    with BertClient(ip=ip, show_server_config=False, check_version=False, check_length=False, mode='NER') as bc:
...
def class_test(ip="localhost", port=5555, port_out=5556):
    with BertClient(ip=ip, port=port, port_out=port_out, show_server_config=False, check_version=False, check_length=False, mode='CLASS') as bc:
        start_t = time.perf_counter()
        ...
        str = '手机还行，但是手机刚开箱时屏幕和背面有很多指纹痕迹，手机壳跟**在地上磨过似的，好几条印子。要不是看在能把这些痕迹擦掉，和闲退货麻烦，就给退了。就不能规规矩矩做生意么。还有送的都是什么吊东西，运动手环垃圾一比，贴在手机后面的固定手环还**是塑料的渡了一层银色，耳机也和图片描述不符，碎屏险已经注册，不知道怎么样。讲真的，要不就别送或者少送，要不，就规规矩矩的，不然到最后还让人觉得不舒服。其他没什么。'
        str2 = '手机是不是正品不确定后台说话是真不要脸。赠品膜没有次只有更次。耳机十元一只的。全******商家买东西真不能贪百二八十的一个耳机就差出来啦要不是双十一错过去啦绝对不能买她家的什么玩意。手机还发热都怀疑是改装的大家别上当**老板**商家最该死'
        str3 = '这款手机我之前就用过，感觉不错，所以就在京东上下单了，准备再入手一部，结果到货之后，手机充电口被使用过应该是退货的机子，找客服人员，客服人员不管，这态度也没谁了'
